...
class Job():
    def __init__(self,HOST_NAME="localhost", user_name = USERNAME, task_path="" ,jobName='Test',
	...
	self.ext = 'for i in `seq 1 ' + str(self.num_times_exe) + '`\ndo\nsingularity exec summa.simg ./runSummaTest.sh $i\ndone'
        self.jobId = None
	...
	num_times_exe = Text(value = str(self.num_times_exe))
        ...
        jobEdits = [jobName,entrance,nNodes,ppn,isGPU,walltime, num_times_exe, confirm]
	    ...
	    #self.summaFolder = summaFolder.value
            ...
            self.exe = 'for i in `seq 1 ' + str(num_times_exe.value) +'`\ndo\nsingularity exec summa.simg ./runSummaTest.sh $i &\ndone\n wait'
	...
	def recursive_download(localPath, remotePath):
	 ...
	 #   out_file_path = filesSelector.getPath()
        ...
        def summa_dir_name():
            stdout = "Found\n"
            ans = "nothing"
            summaTestDirPath = "/data/hsjupyter/summatest"
            while (stdout == "Found\n"):
