...
class SummaSupervisorToHPC(summa_base):
    ...
    def submit(self, params={}):
        ...
        if (self.machine == "keeling"):
            ...
            sjob = SummaKeelingJob(self.workspace_path, self.connection, summa_sbatch, model_source_folder_path,
            ...
            sjob = SummaCometJob(self.workspace_path, self.connection, summa_sbatch, model_source_folder_path,
                                 file_manager_path, name=self.jobname)
            sjob.go()
        return {
    ...
    def connect(self):
        if (self.machine.lower()=="keeling"):
            if (self.username == "cigi-gisolve"):
                self.connection = SSHConnection("keeling.earth.illinois.edu",
            ...
            if self.username=="cigi-gisolve":
                self.connection = SSHConnection("comet.sdsc.edu",
                            user_name="cybergis",
                            key_path=self.private_key_path)
            else:
                self.connection = SSHConnection("comet.sdsc.edu",
    ...
    def job_status(self, remote_id):
        ...
        try:
            out = self.connection.run_command(cmd, line_delimiter=None, raise_on_error=True)
            if out is None:
                return "UNKNOWN"
            # out = \
    ...
    def download(self, remote_model_folder_path, remote_slurm_out_file_path, local_job_folder_path):
        self.connection.download(os.path.join(remote_model_folder_path, "output"),
                                 local_job_folder_path, remote_is_folder=True)
